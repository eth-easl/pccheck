...
class GradientClipper:
    ...
    def step(self, parameters):
        ...
        total_norm, _ = multi_tensor_applier(
            self.multi_tensor_l2norm, self._overflow_buf, [l], False
        )
        ...
        if clip_coef < 1:
            multi_tensor_applier(
                self.multi_tensor_scale, self._overflow_buf, [l, l], clip_coef
            )
...
def main():
    ...
    parser.add_argument(
        "--fp16", default=False, action="store_true", help="Mixed precision training"
    )
    parser.add_argument(
        "--amp", default=False, action="store_true", help="Mixed precision training"
    )
    parser.add_argument(
        "--loss_scale",
        type=float,
    ...
    parser.add_argument(
        "--log_freq", type=int, default=50, help="frequency of logging loss."
    )
    parser.add_argument(
        "--json-summary",
        type=str,
    ...
    parser.add_argument(
        "--use_env", action="store_true", help="Whether to read local rank from ENVVAR"
    )
    parser.add_argument(
        "--skip_checkpoint",
        default=False,
    ...
    parser.add_argument(
        "--bench_total_steps", default=1000, type=int, help="Number of iterations"
    )
    parser.add_argument(
        "--max_async",
        default=1,
        type=int,
        help="Number of concurrent asynchronous checkpoints",
    )
    parser.add_argument(
        "--num_threads", default=1, type=int, help="Number of threads per checkpoint"
    )
    parser.add_argument(
        "--c_lib_path",
        default="",
        type=str,
        help="Path to the PCCHECK checkpoint library",
    )
    ...
    if (
        ...
        and args.do_train
        and os.listdir(args.output_dir) != ["logfile.txt"]
    ):
        print(
            "WARNING: Output directory {} already exists and is not empty.".format(
                args.output_dir
            ),
    ...
    if args.do_train:
        ...
        try:
            ...
            if not args.skip_cache and is_main_process():
                dllogger.log(
                    step="PARAMETER",
                    data={"Cached_train features_file": cached_train_features_file},
                )
        ...
        train_dataloader = DataLoader(
            train_data, sampler=train_sampler, batch_size=args.train_batch_size * n_gpu
        )
        ...
        for epoch in range(1):  # (int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_iter):
                ...
                print(step, args.bench_total_steps)
                if step == args.bench_total_steps:
                    print("DONE!")
                    break
                ...
                if step == warmup or (
                    ...
                    if args.cfreq > 0:
                        if chk is None:
                            ...
                            datasize = (
                                total_size * 2 if args.fp16 is True else total_size * 4
                            )
                            print(f"total_size is {total_size}, datasize is {datasize}")
                            chk = GPMCheckpoint(
                ...
                if step % args.log_freq == 0:
                    dllogger.log(
                        ...
                        data={
                            "step_loss": final_loss,
                            "learning_rate": optimizer.param_groups[0]["lr"],
                        },
                    )

                print(f"Iter {step} took {time.time()-startt}")
                startt = time.time()

        time_to_train = time.time() - train_start
        chk._finish()
        print(
            f"-- BENCHMARK ENDED: Total time: {time_to_train} sec, Number of iterations: {batch}, Number of checkpoints: {checkpoints}"
        )
        print(f"EXECUTION TIME: {time_to_train} sec")
    ...
    if args.do_predict and (args.local_rank == -1 or is_main_process()):
        ...
        dllogger.log(
            step="PARAMETER", data={"predict_batch_size": args.predict_batch_size}
        )
        ...
        answers, nbest_answers = get_answers(
            eval_examples, eval_features, all_results, args
        )
        ...
        if args.max_steps == -1:
            dllogger.log(
                ...
                data={
                    ...
                    "training_sequences_per_second": len(train_features)
                    * args.num_train_epochs
                    / time_to_train,
            ...
            dllogger.log(
                ...
                data={
                    "e2e_train_time": time_to_train,
                    "training_sequences_per_second": args.train_batch_size
                    * args.gradient_accumulation_steps
                    * args.max_steps
                    * gpu_count
                    / time_to_train,
