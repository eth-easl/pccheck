...
from torch.multiprocessing import Manager, Value, Lock
...
class HuggingFaceDatasetWrapper(Dataset):
    ...
    def truncate_and_pad_func(self, input_ids, att_mask, labels, side='right', add_eos=True):
        ...
        if len_pad > 0:
            ...
            pad_inp = torch.empty(
                len_pad, dtype=torch.long).fill_(pad_token_id)
    ...
    def __getitem__(self, idx):
        ...
        # inputs = self.tokenizer(self.hf_dataset[idx])
        inputs = self.hf_dataset[idx]

        input_ids = torch.tensor(inputs["input_ids"])
        att_mask = torch.tensor(inputs["attention_mask"])
        labels = input_ids.clone()

        res = self.truncate_and_pad_func(
            input_ids, att_mask, labels, add_eos=False)
        inputs, labels = res
...
class ModelArguments:
    ...
    c_lib_path: str = field(
        default='',
        metadata={"help": "path to the libtest.so library"}
    )
    max_async: int = field(
        default=1,
        metadata={
    ...
    num_threads: int = field(
        default=1,
        metadata={
            "help": ("Number of CPU threads writing at NVM")
        }
    )
