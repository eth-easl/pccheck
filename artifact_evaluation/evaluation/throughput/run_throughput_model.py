...
def run_vgg():
    ...
    for cf in cfreqs:
        ...
        proc = f"python3.9 {script_dir}/train_checkfreq.py --dataset imagenet  --batchsize {batch_size} --arch vgg16 --cfreq {cf} --bench_total_steps {iters} > vgg16/log_vgg16_cfreq_{cf}.txt"
        os.system(proc)
    ...
    for cf in cfreqs:
        print(f"Checkpoint Frequency {cf}")
        proc = f"python3.9 {script_dir}/train_gpm.py --dataset imagenet  --batchsize {batch_size} --arch vgg16 --cfreq {cf} --bench_total_steps {iters} > vgg16/log_vgg16_gpm_{cf}.txt"
        os.system(proc)
    ...
    for cf in cfreqs:
        print(f"Checkpoint Frequency {cf}")
        proc = f"python3.9 {script_dir}/train_pccheck.py --dataset imagenet  --batchsize {batch_size} --arch vgg16 --cfreq {cf} --bench_total_steps {iters} --max-async 4 --num-threads 2 --psize 8 --c_lib_path {lib_path} > vgg16/log_vgg16_pccheck_{cf}.txt"
        os.system(proc)


def run_opt():
    os.makedirs("opt_13", exist_ok=True)
    script_dir = model_scripts_dir[model]
    batch_size = batch_size_dir[model]
    iters = iters_dir[model]
    ...
    for cf in cfreqs:
        ...
        print(f"Checkpoint Frequency {cf}")
        proc = f"python3.9 {script_dir}/run_clm_cfreq.py --model_name_or_path facebook/opt-1.3b --output_dir output --dataset_name wikitext --dataset_config_name wikitext-2-raw-v1 --do_train --per_device_train_batch_size {batch_size} --cfreq {cf} --bench_total_steps {iters} > opt_13/log_opt13_cfreq_{cf}.txt"
        os.system(proc)
    ...
    for cf in cfreqs:
        os.system("rm -rf output")
        print(f"Checkpoint Frequency {cf}")
        proc = f"python3.9 {script_dir}/run_clm_gpm.py --model_name_or_path facebook/opt-1.3b --output_dir output --dataset_name wikitext --dataset_config_name wikitext-2-raw-v1 --do_train --per_device_train_batch_size {batch_size} --cfreq {cf} --bench_total_steps {iters} > opt_13/log_opt13_gpm_{cf}.txt"
        os.system(proc)
